...
It installs PC A's dedicated, passwordless automation public key in Windows OpenSSH's administrator key file, repairs the file ACL, starts `sshd`, enables a Private-network/LocalSubnet firewall rule, and prints PC B's username, IPv4 addresses, and ED25519 host-key fingerprint. The automation key is restricted by possession and network/firewall scope; its private half never leaves PC A. A direct Ethernet cable can improve reachability but does not replace this authentication setup.
